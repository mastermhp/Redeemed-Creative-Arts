...
export async function sendInvitationEmail(email, inviterName, userType, token, message) {
  try {
    ...
    const mailOptions = {
      ...
      to: email,
      subject: `${inviterName} invited you to join Redeemed Creative Arts ✉️`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
          <meta charset="utf-8">
          <title>You're Invited - Redeemed Creative Arts</title>
        </head>
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
          <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: white; padding: 30px; border: 1px solid #e5e7eb;">
              <h3>Hello!</h3>
              <p><strong>${inviterName}</strong> has invited you to join <strong>Redeemed Creative Arts</strong> as a <strong>${userType}</strong>.</p>
              ${message ? `<p>Message from inviter: "${message}"</p>` : ""}
              <p>Click below to accept the invitation and register:</p>
              <div style="text-align: center; margin: 30px 0;">
                <a href="${invitationLink}" style="display: inline-block; background: #f59e0b; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">
                  Accept Invitation
                </a>
              </div>
              <p>Or copy and paste this link into your browser:</p>
              <p style="word-break: break-all; background: #f3f4f6; padding: 10px; border-radius: 4px;">${invitationLink}</p>
